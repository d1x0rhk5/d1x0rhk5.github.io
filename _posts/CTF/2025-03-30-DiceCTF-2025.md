...
/bake 페이지는 number 값을 받아 current 값에 더한다. `(current < target)` 값이 false가 되면 flag가 나온다. number 값에 `e`와 같은 값을 넣으면 current 값이 NaN이 되고 `(current < target)`가 false가 되어 flag를 살 수 있다.
...
ref를 늘려서 flag를 사는 문제이다. 정상적인 방법으로 `bal` 값을 늘리려면 결국 ref를 늘려야 하는데, 이를 위해서는 10억 번의 request가 필요해 다른 방법을 찾아봤다.

자기 자신의 code를 code로 가지고 있으면 cashout했을 때 `bal` 값이 기하급수적으로 늘어난다. 문제의 /new 페이지는 data 값을 모두 받아야 user를 등록하지만, data를 아직 다 받지 않은 상태에서도 token을 return한다. 따라서 name까지만 보내 token을 받은 뒤 code 값을 계산해 refer에 넣으면 자기 자신을 referer로 삼을 수 있다.
...
위의 코드로 자기 자신이 referer인 계정을 만들었다. 해당 계정의 ref를 2로 만들고 cashout을 반복하면 flag를 살 수 있다.
