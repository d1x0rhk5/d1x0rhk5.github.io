...
3. `no-open-quote`, `open-quote`, `close-quote`를 조합해서 원하는 index의 문자 하나만 렌더링한다.
4. 그 문자의 폭을 `@container` 를 사용해서 후보 집합을 구한다.
5. 같은 문자를 `regular`와 `oblique` 에서 각각 계산하고, 두 후보 집합에 들어있는 값을 구한다.
...
예를 들어 플래그가 `tkb`라고 하면, `sep='""'`일 때 `FLAG.split("").join(sep)`의 결과는 아래처럼 들어간다.

```css
--flag: "t""k""b";
```

CSS 입장에서는 이것이 문자열 토큰의 나열처럼 동작한다. 이 값을 `quotes` 속성에 넣으면 문자 하나하나를 quote string으로 사용할 수 있다.
...
이를 이용해 `i`(인덱스)가 짝수이면 `no-open-quote`를 `i/2`번 쓴 뒤 `open-quote`를 사용하고, `i`가 홀수이면 `no-open-quote`를 `(i+1)/2`번 쓴 뒤 `close-quote`를 사용해서 특정 문자 하나만 렌더링할 수 있었다.
...
`body`의 너비를 `fit-content`로 두면 `body:before`가 실제 문자 하나의 폭을 결정한다. `h1`의 폭을 `100%`로 두면 `h1`의 inline-size가 결국 문자 폭과 같아진다. 이 폭을 아래의 문법 형태로 분류할 수 있다.
...
한 폰트 스타일에는 폭이 같은 글자가 많으므로 같은 글자를 2개의 스타일로 측정해 교집합을 계산한다.

안정성을 위해 폰트 패밀리는 같고 렌더링 스타일만 다른 FreeSans regular와 FreeSans oblique를 사용한다.
...
revenge 문제는 기존 문제에서 sep의 길이를 확인할 때 type을 검사하지 않아 array로 우회할 수 있었기 때문에 나왔다.
