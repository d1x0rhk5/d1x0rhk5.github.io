...
'Hello ' + username 연산 전에 username에 대해 toString을 통한 형변환이 일어난다. 따라서 username.toString을 overwrite하면 에러가 발생한다.
...
sql query를 실행할 때 price_op에 `>/*`, limit 앞부분을 `*/`로 설정하면 쉽게 sql injection이 가능하다.

`information_schema.processlist`를 통해 어떤 쿼리를 실행했는지 확인하면 query 내부의 주석을 볼 수 있다.
...
Limited 1과 같은 소스코드를 사용하는 문제이다. Flag로 시작하는 table의 값을 확인하면 된다.
...
해당 문제의 Mysql은 password를 저장할 때 caching_sha2_password를 사용한다. 비밀번호가 rockyou에 있는 13글자라는 정보를 바탕으로 bruteforce를 진행했다.
...
우선 hash 값을 알아내기 위해 위의 쿼리를 사용했다.
...
php로 만든 파일을 업로드할 수 있는 사이트이다. `../`를 삽입할 방법을 찾아봤지만 불가능했다.

문제에서는 업로드 이후 `chmod -- 000 *` 명령어로 업로드한 파일을 읽을 수 없게 만든다. 하지만 `*`를 사용하면 파일명이 `.`으로 시작하는 파일은 처리되지 않는다.

그리고 `if ($lastDotPosition == false)` 조건문을 사용하는 부분이 있는데, dot의 위치가 맨 앞이면 `$lastDotPosition`이 0이 되어 `$lastDotPosition == false`가 true가 된다.

따라서 `.htaccess` 파일을 사용해 txt 파일을 php 파일처럼 실행하면 flag를 읽어올 수 있다.
