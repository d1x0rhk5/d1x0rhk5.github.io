...
이 때문에 길이가 한계치를 초과해도 `FixedDoubleArray`가 생성되어 내부 버퍼 크기를 벗어나는 상태가 만들어진다.

- JIT 최적화 과정에서 경계 검사 제거
TurboFan JIT 컴파일러의 `simplified-lowering` 단계에는 배열 접근에 대한 경계 검사가 없다.
...
이 단계에서 “index < length”라는 전제 조건 검사를 완전히 빼 버리기 때문에 OOB 쓰기가 가능해진다.
...
그러나 `NewFixedDoubleArray`에서는 최대 길이(`kMaxLength = 67 108 862`) 초과 여부를 검사하지 않으므로 버퍼 오버플로우 상태가 만들어진다.
